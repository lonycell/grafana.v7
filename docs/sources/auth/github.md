...
browser to access ThingSPIN, but with the prefix path of `/login/github`.
When the GitHub OAuth application is created you will get a Client ID and a
Client Secret. Specify these in the ThingSPIN configuration file. For
example:

## Enable GitHub in ThingSPIN
...
Restart the ThingSPIN back-end. You should now see a GitHub login button
...
ThingSPIN instance. For example:
...
your ThingSPIN instance. For example
...
With Team Sync you can map your GitHub org teams to teams in ThingSPIN so that your users will automatically be added to
the correct teams. 
