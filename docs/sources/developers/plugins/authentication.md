...
ThingSPIN has a proxy feature that proxies all data requests through the ThingSPIN backend. The main benefit of using the proxy is secure handling of credentials when authenticating against an external/third-party API. The ThingSPIN proxy also adds [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) headers to the proxied requests.
...
The user saves the API key/password on the plugin config page and it is encrypted (using the `secureJsonData` feature) and saved in the ThingSPIN database. When a request from the data source is made, the ThingSPIN proxy will:

1. Intercept the original request sent from the data source plugin.
1. Load the `secureJsonData` data from the database and decrypt the API key or password on the ThingSPIN backend.
...
When a user saves a password or secret with your data source plugin's Config page, then you can save data in an encrypted blob in the ThingSPIN database called `secureJsonData`. Any data saved in the blob is encrypted by ThingSPIN and can only be decrypted by the ThingSPIN server on the backend. This means once a password is saved, no sensitive data is sent to the browser. If the password is saved in the `jsonData` blob or the `password` field then it is unencrypted and anyone with Admin access (with the help of Chrome Developer Tools) can read it.
...
- then the ThingSPIN proxy will transform the URL from the original request into `https://management.azure.com/foo/bar`
...
This interpolates in data from both `jsonData` and `secureJsonData` to generate the token request to the third-party API. It is common for tokens to have a short expiry period (30 minutes). The ThingSPIN proxy automatically renews the token if it has expired.

## Always restart the ThingSPIN server after route changes

The plugin.json files are only loaded when the ThingSPIN server starts so when a route is added or changed then the ThingSPIN server has to be restarted for the changes to take effect.
