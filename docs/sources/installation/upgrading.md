+++
title = "Upgrade ThingSPIN"
description = "Guide for upgrading ThingSPIN"
keywords = ["grafana", "configuration", "documentation", "upgrade"]
type = "docs"
[menu.docs]
name = "Upgrade ThingSPIN"
...
We recommend that you upgrade ThingSPIN often to stay up to date with the latest fixes and enhancements.
In order to make this a reality, ThingSPIN upgrades are backward compatible and the upgrade process is simple and quick.
...
After you have upgraded it is highly recommended that you update all your plugins as a new version of ThingSPIN
...
Before upgrading it can be a good idea to backup your ThingSPIN database. This will ensure that you can always rollback to your previous version. During startup, ThingSPIN will automatically migrate the database schema (if there are changes or new tables). Sometimes this can cause issues if you later want to downgrade.
...
You can upgrade ThingSPIN by following the same procedure as when you installed it.

#### Upgrade Debian package

If you installed ThingSPIN by downloading a Debian package (`.deb`), then you can execute the same `dpkg -i` command but with the new package. It will upgrade your ThingSPIN installation.
...
If you installed ThingSPIN from our APT repository, then ThingSPIN will automatically update when you run apt-get upgrade to upgrade all system packages.
...
This allows you to upgrade ThingSPIN without risking losing your configuration changes.

### Centos / RHEL

If you installed ThingSPIN by downloading an RPM package you can just follow the same installation guide and execute the same `yum install` or `rpm -i` command but with the new package. It will upgrade your ThingSPIN installation.
...
The dashboard grid layout engine has changed. All dashboards will be automatically upgraded to new positioning system when you load them in v5. Dashboards saved in v5 will not work in older versions of ThingSPIN. Some external panel plugins might need to be updated to work properly.

For more details on the new panel positioning system, [click here]({{< relref "../reference/dashboard.md#panel-size-position" >}})

## Upgrading to v5.2

One of the database migrations included in this release will update all annotation timestamps from second to millisecond precision. If you have a large amount of annotations the database migration may take a long time to complete which may cause problems if you use systemd to run ThingSPIN.
...
If your using ThingSPIN's builtin, LDAP (without Auth Proxy) or OAuth authentication all users will be required to login upon the next visit after the upgrade.
...
If you're embedding ThingSPIN in a `<frame>`, `<iframe>`, `<embed>` or `<object>` on a different website it will no longer work due to a new setting
that per default instructs the browser to not allow ThingSPIN to be embedded. Read more [here]({{< relref "configuration/#allow-embedding" >}}) about
this new setting.
...
If you are using Auth proxy with LDAP an shared cached is used in ThingSPIN so you might want configure [remote_cache] instead. If not
ThingSPIN will fallback to using the database as an shared cache.
...
If you upgrade Elasticsearch to v7.0+ you should make sure to update the data source configuration in ThingSPIN so that version
...
ThingSPIN’s docker image is now based on [Alpine](http://alpinelinux.org) instead of [Ubuntu](https://ubuntu.com/).
...
change the `[security]` setting `cookie_secure` to `true` and use HTTPS when `cookie_samesite` is configured to `none`, otherwise authentication in ThingSPIN won't work properly.

## Upgrading to v7.0

### PhantomJS removed

PhantomJS was deprecated in [ThingSPIN v6.4](https://grafana.com/docs/grafana/latest/guides/whats-new-in-v6-4/#phantomjs-deprecation) and starting from ThingSPIN v7.0.0, all PhantomJS support has been removed. This means that ThingSPIN no longer ships with a built-in image renderer, and we adwise you to install the [ThingSPIN Image Renderer plugin](https://grafana.com/grafana/plugins/grafana-image-renderer).
