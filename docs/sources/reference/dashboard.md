...
A dashboard in ThingSPIN is represented by a JSON object, which stores metadata of its dashboard. Dashboard metadata includes dashboard properties, metadata from panels, template variables, panel queries, etc.
...
| **schemaVersion** | version of the JSON schema (integer), incremented each time a ThingSPIN update brings changes to said schema |
