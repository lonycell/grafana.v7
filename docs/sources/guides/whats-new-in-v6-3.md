+++
title = "What's new in ThingSPIN v6.3"
description = "Feature and improvement highlights for ThingSPIN v6.3"
...
Please read [Using Elasticsearch in ThingSPIN](/features/datasources/elasticsearch/#querying-logs-beta) for more detailed information on how to get started and use it.
...
Please read [Using InfluxDB in ThingSPIN](/features/datasources/influxdb/#querying-logs-beta) for more detailed information on how to get started and use it.
...
For example, lets say a user is removed from an LDAP group. In previous versions of ThingSPIN an admin would have to
wait for the user to logout or the session to expire for the ThingSPIN permissions to update, a process that can take days.

With active sync the user would be automatically removed from the corresponding team in ThingSPIN or even logged out and disabled if no longer
belonging to an LDAP group that gives them access to ThingSPIN.

[Read more](/auth/enhanced_ldap/#active-ldap-synchronization).

### SAML Authentication

Built-in support for SAML is now available in ThingSPIN Enterprise.

[See docs]({{< relref "../auth/saml.md" >}})

### Team Sync for GitHub OAuth

When setting up OAuth with GitHub it's now possible to sync GitHub teams with Teams in ThingSPIN.
