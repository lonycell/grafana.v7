+++
title = "What's new in ThingSPIN v6.0"
description = "Feature and improvement highlights for ThingSPIN v6.0"
...
# What's new in ThingSPIN v6.0

This update to ThingSPIN introduces a new way of exploring your data, support for log data, and tons of other features.

The main highlights are:

- [Explore]({{< relref "#explore" >}}) - A new query focused workflow for ad-hoc data exploration and troubleshooting.
- [ThingSPIN Loki]({{< relref "#explore-and-grafana-loki" >}}) - Integration with the new open source log aggregation system from ThingSPIN Labs.
...
- [Removal of user session storage]({{< relref "#easier-to-deploy-improved-security" >}}) makes ThingSPIN easier to deploy and improves security.
...
ThingSPIN's dashboard UI is all about building dashboards for visualization. **Explore** strips away all the dashboard and panel options so that you can focus on the query and metric exploration. Iterate until you have a working query and then think about building a dashboard. You can also jump from a dashboard panel into **Explore** and from there do some ad-hoc query exporation with the panel queries as a starting point.
...
**Explore** is a new paradigm for ThingSPIN. It creates a new interactive debugging workflow that integrates two pillars
...
functions to your query. There is also integration between Prometheus and ThingSPIN Loki (see more about Loki below) that
...
aggregation system from ThingSPIN Lab called [ThingSPIN Loki](https://github.com/grafana/loki).
...
Read more about ThingSPIN Loki [here](https://github.com/grafana/loki) or [ThingSPIN Labs hosted Loki](https://grafana.com/loki).
...
ThingSPIN v6.0 has a completely redesigned UX around editing panels. You can now resize the visualization area if you want
...
A major part of all the work that has gone into ThingSPIN v6.0 has been on the migration to React. This investment
is part of the future-proofing of ThingSPIN's code base and ecosystem. Starting in v6.0 **Panels** and **Data
...
Built-in support for [Google Stackdriver](https://cloud.google.com/stackdriver/) is officially released in ThingSPIN 6.0. Beta support was added in ThingSPIN 5.3 and we have added lots of improvements since then.

To get started read the guide: [Using Google Stackdriver in ThingSPIN](/features/datasources/stackdriver/).

## Azure Monitor data source

One of the goals of the ThingSPIN v6.0 release is to add support for the three major clouds. Amazon CloudWatch has been a core data source for years and Google Stackdriver is also now supported. We developed an external plugin for Azure Monitor last year and for this release the [plugin](https://grafana.com/plugins/grafana-azure-monitor-datasource) is being moved into ThingSPIN to be one of the built-in data sources. For users of the external plugin, ThingSPIN will automatically start using the built-in version. As a core data source, the Azure Monitor data source is able to get alerting support, in the 6.0 release alerting is supported for the Azure Monitor service, with the rest to follow.

The Azure Monitor data source integrates four Azure services with ThingSPIN - Azure Monitor, Azure Log Analytics, Azure Application Insights and Azure Application Insights Analytics.

Please read [Using Azure Monitor in ThingSPIN documentation](/features/datasources/azuremonitor/) for more detailed information on how to get started and use it.

## Provisioning support for alert notifiers

ThingSPIN now has support for provisioning alert notifiers from configuration files, allowing operators to provision notifiers without using the UI or the API. A new field called `uid` has been introduced which is a string identifier that the administrator can set themselves. This is the same kind of identifier used for dashboards since v5.0. This feature makes it possible to use the same notifier configuration in multiple environments and refer to notifiers in dashboard json by a string identifier instead of the numeric id which depends on insert order and how many notifiers exist in the instance.

## Easier to deploy and improved security

ThingSPIN 6.0 removes the need to configure and set up additional storage for [user sessions](/tutorials/ha_setup/#user-sessions). This should make it easier to deploy and operate ThingSPIN in a
...
Named colors also enables ThingSPIN to adapt colors to the current theme.
