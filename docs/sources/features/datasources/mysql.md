+++
title = "Using MySQL in ThingSPIN"
description = "Guide for using MySQL in ThingSPIN"
...
# Using MySQL in ThingSPIN

> Only available in ThingSPIN v4.3+.
>
> Starting from ThingSPIN v5.1 you can name the time column *time* in addition to earlier supported *time_sec*. Usage of *time_sec*  will eventually be deprecated.

ThingSPIN ships with a built-in MySQL data source plugin that allows you to query and visualize
...
*Max lifetime* | The maximum amount of time in seconds a connection may be reused, default `14400`/4 hours. This should always be lower than configured [wait_timeout](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_wait_timeout) in MySQL (ThingSPIN v5.4+).
...
> Only available in ThingSPIN v5.4+.
...
In the FROM field, ThingSPIN will suggest tables that are in the configured database. To select a table or view in another database that your database user has access to you can manually enter a fully qualified name (database.table) like `otherDb.metrics`.
